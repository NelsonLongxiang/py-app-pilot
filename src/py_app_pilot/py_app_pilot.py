...
from PyQt5.QtWidgets import (
    ...
    QLabel, QLineEdit, QMessageBox, QFileDialog,QMenu
)
...
class PythonAppManager(QMainWindow):
    ...
    def init_settings_ui(self):
        ...
        # 应用选择提示
        self.settings_label = QLabel("请在应用管理中选择一个应用进行设置")
    ...
    def init_system_management_ui(self):
        ...
        # 加载全局Python路径
        self.global_python_path_input.setText(self.global_settings.get('python_path', sys.executable))
    ...
    def on_app_selected(self):
        ...
        if current_item:
            ...
            # 连接编辑完成信号到保存方法
            self.args_input.editingFinished.connect(self.save_app_settings)
            self.cwd_input.editingFinished.connect(self.save_app_settings)
        else:
            self.settings_group.setEnabled(False)
    ...
    def start_app(self,current_item=None):
        ...
        python_path = app_python_path if app_python_path else self.global_settings.get('python_path', sys.executable)
        ...
        try:
            ...
            output_reader.finished.connect(lambda pid=process.pid, app_path=app.path: self.on_process_finished(pid, app_path))
